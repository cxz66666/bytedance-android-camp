...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        notesAdapter = new NoteListAdapter(new NoteOperator() {
            ...
            public void deleteNote(Note note) {
                ...
                int rows=database.delete(TodoNote.TABLE_NAME,TodoNote._ID+"=?",new String[]{note.id+""});
                if(rows>0){
            ...
            public void updateNote(Note note) {
                ...
                int rows=database.update(TodoNote.TABLE_NAME,values,TodoNote._ID+"=?",new String[]{note.id+""});
                if(rows>0){
                    notesAdapter.refresh(loadNotesFromDatabase());
                }
            }
    ...
    private List<Note> loadNotesFromDatabase() {
        ...
        //  7. 此处query数据库数据
        String sortOrder= SORT_ORDER_PRIORITY_AND_ID;
